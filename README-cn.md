...
* **_车载交流补偿(In-Car Communication, ICC)系统 (2021 - 至今)_**

    * 反馈（啸叫）控制算法
    * 超低延迟单通道降噪算法
    * 工程化落地

    为广汽等客户开发相关功能。车内交流补偿通过车载音频系统中的麦克风与扬声器实现实时的本地扩音，以解决高速行驶中噪声较大、或前后排沟通时语音声压级较低等问题。设计能够在12毫秒的端到端时延约束下，满足大部分ITU-T P.1150规范的要求，并在 ADSP-21571 平台（单核450MHz）落地。
