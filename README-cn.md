...
* **_车载交流补偿(In-Car Communication, ICC)系统 (2021 - 至今)_**
    ...
    * 工程化落地
    ...
    为广汽等客户开发相关功能。车内交流补偿通过车载音频系统中的麦克风与扬声器实现实时的本地扩音，以解决高速行驶中噪声较大、或前后排沟通时语音声压级较低等问题。车内本地扩音需要解决(优化)三个问题：1）
    前向通道的声压级提升量；2）车内噪声的抑制；以及 3）前向通道的时延。较高的前向通道增益容易让此闭环系统失稳产生啸叫，故使用最大稳定增益提升量(MSG)衡量反馈控制的性能。车内噪声除了平稳的发动机、空调噪声外，还包括鸣笛等非平稳噪声需要处理。最后，根据心理声学的研究结果，系统的端端时延需控制在 15 毫秒以内，否则 ICC 算法反而会加重乘员的沟通负担。  

    设计了一种端到端时域模型以解决单通道低时延降噪的问题，通过时域卷积来降低时延，同时时域信号的过采样能满足一定的自由度，根据噪声平稳与非平稳的特性，采用双支路的结构，一路采用 RNN 类模型解决平稳噪声，一路采用多帧卷积结构解决非平稳噪声，同时利用 Unet的 en-decoder 结构减低整体模型的算力，再结合量化训练，整体算力可低至 150M/s 及以下，在性能上较 Baseline 版本，POLQA、PESQ、STOI 等指标上相对提升 26%以上。系统在 ADSP-21571 平台（450MHz）的负载率小于 40%，处理时延为 7 毫秒，端端时延能够控制在 12 毫秒以内。


* **_开发适用于嵌入式音频产品的轻量级语音活动检测(VAD)系统 (2020 - 2020)_**
...
* **_开发适用于嵌入式音频产品的轻量级关键词唤醒器(KWS) (2019 - 2020)_**
...
* **_开发三星 Galaxy Home 智能音箱的全自动性能测试 (2018 - 2019)_**
...
* **_开发基于深度神经网络的单麦克风降噪算法 (2017 - 2018)_**  
...
* **_开发新型功放电源轨迹跟踪方法_** (2014 - 2015)

    * 创新的H级电源轨迹跟踪电路
    * 节省DAC成本
    * 提高了车载功放部门的利润

* **_领导开发ACOspector(TM) – 非侵入式液体性质实时测量仪_** (2010 - 2014)
